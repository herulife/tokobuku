...
def main():
    ...
    try:
        ...
        files = [
            ("d:/uma/cintabuku/backend/src/middlewares/uploadMiddleware.ts", "/tmp/uploadMiddleware.ts"),
            ("d:/uma/cintabuku/backend/src/index.ts", "/tmp/index.ts")
        ]
        ...
        cmd_move = f"echo '{VPS_PASS}' | sudo -S bash -c 'mv /tmp/uploadMiddleware.ts /home/ubuntu24/my-docker-apps/apps/cintabuku/backend/src/middlewares/uploadMiddleware.ts && mv /tmp/index.ts /home/ubuntu24/my-docker-apps/apps/cintabuku/backend/src/index.ts'"
        stdin, stdout, stderr = ssh.exec_command(cmd_move)
        print(stdout.read().decode())
        
        print("Rebuilding backend...")
        # Rebuild backend ONLY
        cmd_rebuild = f"echo '{VPS_PASS}' | sudo -S bash -c 'cd /home/ubuntu24/my-docker-apps && docker compose up -d --build --force-recreate cintabuku-backend'"
        ...
        print("Backend deployment finished.")
        ssh.close()
    except Exception as e:
        print(f"Error: {e}")
        sys.exit(1)
